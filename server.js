...
app.post('/vouchers/generate', (req, res) => {
  ...
  for (let i = 0; i < toCreate; i++) {
    const code = `${t.prefix}-${pad(existing + i + 1)}`;
    if (!store.vouchers[code]) store.vouchers[code] = { tier, status: 'unused', branch: '', date: '', time: '', amount: 0 };
  }
 
  res.json({ ok: true, created: toCreate, from: `${t.prefix}-${pad(existing+1)}`, to: `${t.prefix}-${pad(t.limit)}` });
});
